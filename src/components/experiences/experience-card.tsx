...
export function ExperienceCard({ experience }: { experience: any }) {
  return (
    <Link href={`/dashboard/experiences/${encodeURIComponent(experience.company.name)}/${experience.id}`} className="block h-full group">
      <Card className="bg-white dark:bg-white/5 backdrop-blur-xl border-border/80 dark:border-white/10 shadow-md dark:shadow-none group-hover:shadow-lg dark:group-hover:shadow-none transition-all flex flex-col h-full">
        <CardHeader className="pb-3">
          <div className="flex justify-between items-start gap-4">
            <div className="space-y-1">
              <CardTitle className="text-xl leading-tight group-hover:text-primary transition-colors">{experience.title}</CardTitle>
